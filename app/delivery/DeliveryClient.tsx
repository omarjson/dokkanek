        ...
        <Card key={t.id}>
          <div className="flex flex-wrap justify-between gap-2">
            <div>
              ...
              <div className="text-xs text-slate-500">المندوب: {t.courierName || "—"} • {fmtDate(t.date)}</div>
            </div>
            <div className="text-sm text-end">
              <div>الإجمالي {lyd(t.sale.total)} • المحصل {lyd(t.collected)} • المتبقي {lyd(t.sale.total - t.sale.paid)}</div>
              <div className="flex flex-wrap gap-1 mt-1">
                <button className={btnXsCls} onClick={() => patch(t.id, { status: "WITH_COURIER" })}>بحوزة المندوب</button>
                <button className={btnXsCls} onClick={() => patch(t.id, { status: "DELIVERED" })}>تم التسليم</button>
                <button className={btnXsCls} onClick={() => patch(t.id, { status: "FAILED" })}>تعذر</button>
              </div>
              <div className="flex flex-wrap gap-1 mt-1">
                ...
                <button className={btnXsCls} onClick={() => patch(t.id, { collected: Number(collect[t.id] || 0) })}>تحصيل</button>
              </div>
            </div>
          </div>
        </Card>
