...
export function ProductsTable({ products }: { products: P[] }) {
  ...
  const [editing, setEditing] = useState<null | { id: string; name: string; costPrice: string; salePrice: string; barcode: string; minQuantity: string }>(null);
            ...
            <tr key={p.id} className="border-t hover:bg-slate-50">
              ...
              <td className="p-2">
                <div className="flex flex-wrap gap-1">
                  <button className={btnXsCls} onClick={() => setEditing({ id: p.id, name: p.name, costPrice: String(p.costPrice), salePrice: String(p.salePrice), barcode: p.barcode, minQuantity: String(p.minQuantity) })}>
                    تعديل
                  </button>
